...
def BingTranslate(api_key, filtered_sent, language_from, language_to):
    ...
    for sent in filtered_sent.keys():
        ...
        if dict_name.get(sent1, -1) != -1:
            ...
            result = response[0]["translations"][0]["text"].replace('&#39;', "'").replace('&quot;', "'")
            translation_dic[sent1] = result
            ...
            for ii in range(lenn + 1):
                ...
                try:
                    request = requests.post(constructed_url, params=params, headers=headers, json=body1)
                except:
                    time.sleep(100)
                    request = requests.post(constructed_url, params=params, headers=headers, json=body1)
                response = request.json()
                ...
                for i in range(len(body1)):
                    try:
                        dict_name[filtered_sent[sent][ii * 50 + i]] = response[i]["translations"][0]["text"].replace(
                            '&#39;', "'").replace('&quot;', "'")
                    except KeyError:
                        pass
                for i in range(len(body1)):
                    try:
                        translation_dic[filtered_sent[sent][ii * 50 + i]] = response[i]["translations"][0][
                            "text"].replace(
                            '&#39;', "'").replace('&quot;', "'")
                    except KeyError:
                        pass
...
def YoudaoTranslate(filtered_sent, source_language, target_language):
    ...
    for sent in filtered_sent.keys():
        sent1 = sent.split("\n")[0]
        result = youdao_translate(source_sentence=sent1,source_language=source_language,target_language=target_language)
        translation_dic[sent1] = result
        for new_s in filtered_sent[sent]:
            result = youdao_translate(source_sentence=new_s,source_language=source_language,target_language=target_language)
            translation_dic[new_s] = result
...
# 请求不能高于每秒1次
def baidu_translate(source_sentence, source_language, target_language):
    """
    百度翻译接口
    :param source_sentence: 源语句
    :param source_language: 源语言
    ...
    if list(source_sentence)[0] in lower_case:
        ...
        my_url = api_url + '?appid=' + str(
            my_appid) + '&q=' + source_sentence + '&from=' + 'en' + '&to=' + 'zh' + '&salt=' + salt + '&sign=' + final_sign
    else:
        paramas = {
            'q': source_sentence,
            'from': source_language,
        ...
        my_url = api_url + '?appid=' + str(
            my_appid) + '&q=' + source_sentence + '&from=' + 'zh' + '&to=' + 'en' + '&salt=' + salt + '&sign=' + final_sign
