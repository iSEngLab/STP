...
import os
import openpyxl
ssss=['Politics', 'Business', 'Culture', 'Sports', 'Tech', 'TravelFood', 'Health', 'LifeStyle', 'Legal', 'Opinion']
...
class Pair:
    def __init__(self,source,gen,transl,transgl,error,serror,gerror,distance,distance_word):
        self.source=source
        self.gen=gen
        self.transl=transl
        self.trangl=transgl
...
def out():
    ...
    #
    ssss = ['Politics', 'Business', 'Culture', 'Sports', 'Tech', 'TravelFood', 'Health', 'LifeStyle', 'Legal',
            'Opinion']
    software = ["Google", "Bing"]
    result_all_dic={}
            ...
            for i in range(1, sheet1.nrows - 1):
                ...
                if type == 1:
                    ...
                    if len(error1) > 0 or len(error2) > 0:
                        flag = True
                    ListPair.append(
                        Pair(Result[result1[0]], Result[result2[0]], "", "", flag, error1, error2,0,0))

            ccc = 0
            i11 = 0
            name_result = ll + sheet1.name
...
def STP(dataset,software,ErrorShow=False,ErrorType=False):
                    ...
                    for new in gen:
                        ...
                        for old in LLL:
                            ...
                            if flag1 == True:
                                ...
                                flag = False
                                if len(Result[old].error) > 0 or len(Result[new].error) > 0:
                                    flag = True
                                ListPair.append(
                                    Pair(old, new, Result[old].trans, Result[new].trans, flag, Result[old].error,
                                         Result[new].error, distance, worddistance))
                ...
                for word_distance_en in range(1, 2):
                    ...
                    for i in ListPair:
                        ...
                        if i.error == True:
                            if i.distance not in PairErrorDict.keys():
                                ...
                                PairErrorDict[i.distance] = PairErrorDict[i.distance] + 1
                    # print(ll + "Pair error:" + str(ccc2) + " count:" + str(i22))
                    # print("原句错误数量：" + str(len(listYuan)))
                    ...
                    for dis in range(left_index, right_index + 1):
                        ...
                        if count_pair != 0:
                            ...
                            result_dic[dis] = res_precent + "(%d/%d)" % (count_error, count_pair)
                        else:
                            res_precent = '{:.1%}'.format((count_error / count_pair))
                            result_dic[dis] = res_precent + "(%d/%d)" % (count_error, count_pair)
    ...
    if ErrorShow==True or ErrorType==True:
...
def Overlap():
    ...
    rti_stp_pat=len(list(set(PatInvGoogleList).intersection(RTIGoogleList,STPGoogleList)))
    stp_pat=len(list(set(STPGoogleList).intersection(PatInvGoogleList)))
    result= "RTI :"+str(rti)+"STP:" +str(stp)+"PatInv:"+str(pat)+"rti_stp:"+str(rti_stp-rti_stp_pat)+"rti_patinv"+str(rti_Pat-rti_stp_pat)+"stp_pat"+str(stp_pat-rti_stp_pat)+"stp_pat_rti"+str(rti_stp_pat)
    print(result)
    print("bing")
    ...
    result = "RTI :" + str(rti) + "STP:" + str(stp) + "PatInv:" + str(pat) + "rti_stp:" + str(
        rti_stp - rti_stp_pat) + "rti_patinv" + str(rti_Pat - rti_stp_pat) + "stp_pat" + str(
        stp_pat - rti_stp_pat) + "stp_pat_rti" + str(rti_stp_pat)
    print(result)
